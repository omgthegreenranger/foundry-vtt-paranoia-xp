...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'paranoia', 'Paranoia', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.paranoia/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.paranoia/g),
   ...
   * Rename files.
   *
   * Rename files that had paranoia in their name, such as
   * css/paranoia.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*paranoia*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('paranoia', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-paranoia-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Paranoia's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace paranoia name mentions in files.
      generator.replaceFileContents();
      // Rename files that had paranoia in their name.
